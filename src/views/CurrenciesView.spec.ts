import { fetchRateHistory, fetchRates, fetchRatesOn } from '@/services/exchangeRates';
...
describe('CurrenciesView', () => {
  ...
  it('returns to latest rates and clears the date query param via the back-to-latest button', async () => {
    ...
    const backButton = wrapper.findAll('button').find((button) => button.text() === 'Back to latest');
